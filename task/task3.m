...
% aus Unterlagen, Ausgangs 
N = 5;

% method 2, more advisable

Base_case    = changeRxnBounds(Gluc_BC,'EX_glc__D_e',-7.3,'l')   ;
Base_case    = changeRxnBounds(Gluc_BC,'EX_glc__D_e',-7.3,'u')   ;
% nh4 exchange vector
exchangeRatesNH4 = linspace(0.08,0.9,10);
...
for k=1:numel(exchangeRatesNH4)
    ...
    var_Name = strcat('nh4_',num2str(exchangeRatesNH4(k)));
    % adjust problem boundaries    
    interm   = changeRxnBounds(Base_case,'EX_nh4_e',exchangeRatesNH4(k)*(-1),'l');
    % solve Problem
    S_interm = optimizeCbModel(interm) 
    %create table row with summary
    [T_row1] = createRelevantOutput(Gluc_BC,S_Gluc_BC,"Glucose")	;
    % clean space
    variable.(var_Name) = "here cones the T_Row_x Variable which contains the solution";
